...
- Bug reports should include a crash screenshot and reproducible reproduction steps or trigger conditions, matching `.github/ISSUE_TEMPLATE/`.
...
- **Do not use `love . --test` to verify ranked/net mode changes.** The smoke scan only exercises local modes (solo, co-op, versus with bots). Ranked matchmaking uses WebSocket action codes 1400–1414 over a live server connection (WS actions `match_join`, `match_leave`, `match_found`, `match_start_ranked`, `match_finish_ranked`, `auth_snapshot`, `input_ack`, `rollback_trigger`) and rollback netcode (`parts/player/rollback.lua`). These are completely disconnected during `--test` because there is no connected server. Ranked-specific bugs — desync, rollback failures, input buffer overflow, authoritative snapshot mismatches — will **never** surface under `--test`. Use live multiplayer: pair two clients against each other or connect to the running GameServer instance. See Teblocks-GameServer/AGENTS.md for server-side testing guidance.
