...
function scene.draw()
    ...
    gc_translate(400,150)
        ...
        if NET.matchmaking then
            ...
            gc_printf(("Search time: %.1fs"):format(NET.searchTimer),0,120,480,'center')
        else
...
scene.widgetList={
    WIDGET.newKey{name='match',      x=640,y=520,w=360,h=90,font=38,color='lG',
        ...
        hideF=function() return not USER.uid or NET.matchFoundPending end},
    WIDGET.newButton{name='back',    x=1140,y=640,w=170,h=80,sound='back',font=60,fText=CHAR.icon.back,code=pressKey'escape',
        hideF=function() return NET.matchFoundPending end},
}
